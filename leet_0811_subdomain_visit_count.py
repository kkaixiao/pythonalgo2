...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            ...
            tempStr = cpdomain[1][len(cpdomain[1]) - 1]

            for i in range(len(cpdomain[1]) - 2, -2, -1):
                if tempStr in domainDict:
                    domainDict[tempStr] += int(cpdomain[0])
                else:
                    domainDict[tempStr] = int(cpdomain[0])

                if i >= 0:
                    tempStr = cpdomain[1][i] + '.' + tempStr

        res = []

        for element in domainDict:
            res.append(str(domainDict[element]) + " " + element)
        return res
