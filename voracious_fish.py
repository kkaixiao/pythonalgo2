...
def fish(arr_weight, arr_direction):
    survivors = 0
    stack = []
    for i in range(len(arr_weight)):
        one_weight = arr_weight[i]
        if arr_direction[i] == 1:
            stack.append(one_weight)
        else:
            if stack:
                weight_down = stack.pop()
            else:
                weight_down = -1

            while weight_down != -1 and weight_down < one_weight:
                if stack:
                    weight_down = stack.pop()
                else:
                    weight_down = -1
...
# 0 represents left-going or upstream, 1 represents right-going or downstream
arr2= [0, 1, 0, 0, 0]


print(fish(arr1, arr2))