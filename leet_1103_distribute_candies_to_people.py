...
class Solution:
    def distributeCandies(self, candies: int, num_people: int) -> List[int]:
        ...
        nLeast = math.floor(math.sqrt(candies * 2))
        if nLeast * (nLeast + 1) < candies * 2:
            nLeast += 1

        if nLeast % num_people == 0:
            rounds = nLeast // num_people
        else:
            rounds = nLeast // num_people + 1

        last = int(nLeast % num_people - 1)
